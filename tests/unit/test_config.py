...
def test_defaults(monkeypatch):
    monkeypatch.delenv("SENTRY_MODE_CONFIG", raising=False)
...
def test_environment_overrides_yaml(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SENTRY_MODE_CAMERA__WIDTH", "1280")
    monkeypatch.setenv("SENTRY_MODE_CAMERA__DEVICE", "2")
    monkeypatch.setenv("SENTRY_MODE_LOGGING__LEVEL", "DEBUG")
...
def test_config_env_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("SENTRY_MODE_CONFIG", str(path))
