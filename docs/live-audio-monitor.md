...
hidden tab. When satellites have microphones, a list next to it chooses which one to hear
(`GET /api/microphones`); the choice is remembered in the browser.

## How it sounds close to live

GET `/api/audio/monitor` streams raw 16-bit mono PCM at 16 kHz from `pw-record`, or with
`?source_id=` from a [satellite microphone](satellites.md#microphones), in the same format. The page
...
Two listeners can hear each microphone at once; a third is refused. Browsers keep a page silent
