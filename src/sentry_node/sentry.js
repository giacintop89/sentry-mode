(() => {
  ...
  function options(select, entries, selected) {
    ...
    for (const [value, label] of entries) { const o=document.createElement('option'); o.value=value;o.textContent=label;select.add(o); }
  ...
  function renderRules() {
    ...
    config.rules.forEach((rule, index) => {
      const row=document.createElement('div');row.className='rule';const text=document.createElement('div');
      ...
      text.append(title,detail);const edit=document.createElement('button');edit.className='secondary';edit.textContent='Edit';
      edit.addEventListener('click',()=>{if(dirty.has('rule'))return message('Save the current rule or reset the editor first.','error');loadRule(index);});row.append(text,edit);$('rules').append(row);
    });locks();
  }
  ...
  function loadRule(index) {
    ...
    $('rule-heading').textContent=index<0?'New rule':'Edit rule';
    ...
    $('rule-text').value=t?.text||'';$('rule-voice').value=t?.voice||'en';$('rule-rate').value=t?.rate||175;
    ...
    dirty.delete('rule');showActionOptions();locks();
  }
  ...
  $('preview').addEventListener('click',async()=>{
    if(video?.running)return action('video/stop');
    await action('video/start');
    if(video?.running)location.href='/';
  });
  async function poll() {
    try {
      ...
      $('preview').textContent=video.running?'Hide live preview':'Show live preview';
  ...
  async function init(){try{const data=await api('sentry/config');config=data.config;revision=data.revision;
    ...
    options($('rule-object'),data.objects.map(x=>[x,x]));refreshCommands();renderRules();loadRule(config.rules.length?0:-1);loadCommand('');await poll();message(data.error||'',data.error?'error':'');
