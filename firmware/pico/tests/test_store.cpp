...
std::string framed(const char* payload, uint32_t sequence) {
  char buffer[512];
  size_t size = write_record(payload, std::strlen(payload), sequence, buffer, sizeof(buffer));
...
TEST(what_was_written_is_what_is_read_back) {
  ...
  Record record;
  CHECK(read_record(slot.data(), slot.size(), record));
...
TEST(a_write_that_stopped_halfway_is_not_a_configuration) {
  ...
  for (size_t kept = 0; kept < whole.size(); ++kept) {
    Record record;
    if (read_record(whole.data(), kept, record)) {
...
TEST(a_byte_that_changed_after_the_write_is_not_a_configuration) {
  ...
  for (size_t index = 0; index < whole.size(); ++index) {
    ...
    Record record;
    if (read_record(damaged.data(), damaged.size(), record)) {
...
TEST(an_erased_slot_is_not_mistaken_for_an_empty_configuration) {
  ...
  Record record;
  CHECK(!read_record(erased.data(), erased.size(), record));
  CHECK(!read_record(blank.data(), blank.size(), record));
}
...
TEST(the_newer_of_the_two_slots_is_the_one_that_boots) {
  ...
  Record record;
  CHECK(choose(older.data(), older.size(), newer.data(), newer.size(), record) == Slot::kSecond);
  CHECK(record.sequence == 5);
  CHECK(choose(newer.data(), newer.size(), older.data(), older.size(), record) == Slot::kFirst);
  CHECK(record.sequence == 5);
}
...
TEST(an_interrupted_write_leaves_the_previous_configuration_in_charge) {
  ...
  Record record;
  CHECK(choose(good.data(), good.size(), torn.data(), torn.size(), record) == Slot::kFirst);
...
TEST(a_node_with_nothing_valid_in_either_slot_says_so) {
  std::string erased(64, static_cast<char>(0xFF));
  Record record;
  CHECK(choose(erased.data(), erased.size(), erased.data(), erased.size(), record) ==
        Slot::kNeither);
}
...
TEST(a_sequence_number_that_wrapped_round_is_still_newer) {
  ...
  Record record;
  CHECK(choose(old_high.data(), old_high.size(), wrapped.data(), wrapped.size(), record) ==
        Slot::kSecond);
...
TEST(a_configuration_larger_than_the_node_can_hold_is_refused_before_it_is_written) {
  ...
  CHECK(write_record(huge.data(), huge.size(), 1, buffer, sizeof(buffer)) == 0);
  char small[8];
  CHECK(write_record(kConfiguration, std::strlen(kConfiguration), 1, small, sizeof(small)) == 0);
}
